...
<div class="attendance-table">
    ...
    <div class="attendance-table__month-nav">
        <a href="{{ route('attendances.index', ['date' => $targetDate->copy()->subMonth()->format('Y-m')]) }}" class="attendance-table__month-nav--prev">
            <span class="arrow">←</span>前月
        </a>
        <span class="attendance-table__month-nav--current">{{ $targetDate->format('Y/m') }}</span>
        <a href="{{ route('attendances.index', ['date' => $targetDate->copy()->addMonth()->format('Y-m')]) }}" class="attendance-table__month-nav--next">
            ...
            @foreach($datesInMonth as $date)
                ...
                <tr class="attendance-table__row">
                    ...
                    <td class="attendance-table__body--detail">
                        @if($attendance)
                            <a href="{{ route('attendances.detail', $attendance->id) }}">詳細</a>
                        @endif
                    </td>
                </tr>
            @endforeach
